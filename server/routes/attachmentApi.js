    ...
    try {
      ...
      const handleUploadData = async (data) => {
        try {
          ...
          // Create attachment via server writeAsync
          const uploader = await Attachments.writeAsync(
            fileBuffer,
            {
              fileName: fileName,
      ...
      // If req.body was already parsed by bodyParser middleware, process it immediately
      if (req.body && typeof req.body === 'object' && Object.keys(req.body).length > 0) {
        clearTimeout(timeout);
        return await handleUploadData(req.body);
      }
